...
mod tests {
    ...
    #[test]
    fn malformed_assets_and_limits_fail_closed() {
        assert!(validate_release_root(&format!("https://{}", "x".repeat(2049))).is_err());
        ...
        .unwrap();
        assert_eq!(fs::read(copied).unwrap(), b"asset");
        assert!(download_file("file:///unused", &directory.path().join("zero"), 0).is_err());
        assert!(
            download_file(
                "file:///unused",
                &directory.path().join("huge"),
                MAX_ARCHIVE_BYTES + 1,
            )
            .is_err()
        );
        ...
        assert!(read_limited(&empty, 1).is_err());
        assert!(read_limited(&source, 4).is_err());
        assert!(parse_checksum("not-a-checksum").is_err());
        assert!(sha256_file(&source, 4).is_err());
        assert!(open_regular_no_follow(directory.path()).is_err());
    }

    #[test]
    fn digest_mismatch_and_bounded_child_failures_are_reported() {
        let directory = tempfile::tempdir().unwrap();
        let archive = directory.path().join("archive");
        let checksum = directory.path().join("archive.sha256");
        fs::write(&archive, b"bytes").unwrap();
        fs::write(&checksum, format!("{}  archive\n", "0".repeat(64))).unwrap();
        let mut signed = manifest(b"bytes");
        signed.sha256 = "0".repeat(64);
        assert!(verify_archive(&archive, &checksum, &signed).is_err());

        let mut failed = Command::new("/bin/sh");
        failed.args(["-c", "exit 7"]);
        assert!(bounded_status(&mut failed, "fixture failure", Duration::from_secs(1)).is_err());

        let mut slow = Command::new("/bin/sh");
        slow.args(["-c", "sleep 1"]);
        assert!(bounded_status(&mut slow, "fixture timeout", Duration::ZERO).is_err());
    }
}
