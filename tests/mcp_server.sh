...
case "$remote" in
  ...
  *'scontrol '*)
    ...
    ;;
  *'sstat '*) : ;;
  *'sinfo '*) printf 'gpu*|up|idle|8|gpu:1\n' ;;
  *)
...
#!/bin/sh
printf 'gpu*|up|idle|8|gpu:1\n'
EOF
chmod 755 "$fake_bin/squeue" "$fake_bin/scontrol" "$fake_bin/sstat" \
    "$fake_bin/sbatch" "$fake_bin/scancel" "$fake_bin/tmux" "$fake_bin/ssh" "$fake_bin/sinfo"
...
test "$(printf '%s\n' "$tools" | grep -o '"name":"slurm_' | wc -l)" -ge 11
...
printf '%s\n' "$script_result" | grep -F '1 scripts (1 shown)' >/dev/null
...
request '{"jsonrpc":"2.0","id":220,"method":"tools/call","params":{"name":"slurm_doctor","arguments":{}}}' | grep -F '"ok":true' >/dev/null
request '{"jsonrpc":"2.0","id":221,"method":"tools/call","params":{"name":"slurm_refresh_bank","arguments":{}}}' | grep -F '"ok":true' >/dev/null
request '{"jsonrpc":"2.0","id":222,"method":"tools/call","params":{"name":"slurm_explain_pending","arguments":{"cluster":"alpha","job_id":"123"}}}' | grep -F '"ok":true' >/dev/null
request '{"jsonrpc":"2.0","id":223,"method":"tools/call","params":{"name":"slurm_adopt_job","arguments":{"cluster":"alpha","job_id":"123","expected_job_name":"mcp-train"}}}' | grep -F '"adopted":true' >/dev/null
request '{"jsonrpc":"2.0","id":224,"method":"tools/call","params":{"name":"slurm_wait_job","arguments":{"cluster":"alpha","job_id":"123","timeout_seconds":1,"poll_interval":1,"until":["state_change"]}}}' | grep -F '"ok":true' >/dev/null
